...
h <- function(WB,al,y0,z0) {
  
f <- function(B,WB,al,y0,z0) {
  Bp5 <- B[1]
  B0 <- B[2]
  Bnp5 <- B[3]
  -WB*((1/(1+exp(al - B0)))*(1/(1+exp(al + y0 - Bnp5))) + (1/(1+exp(al - B0)))*(1/(1+exp(al + z0 - Bp5))) + (1/(1+exp(al + z0 - Bp5)))*(1/(1+exp(al + y0 - Bnp5))) - 2*(1/(1+exp(al - B0)))*(1/(1+exp(al + y0 - Bnp5)))*(1/(1+exp(al + z0 - Bp5)))) + B0 + Bp5 + Bnp5
}   

al = 0
y0=-.5
o <- optim(c(1,1,1,WB,al,y0,z0),function(B) f(B,WB,al,y0,z0),gr=NULL,method = "L-BFGS-B", lower = c(0,0,0), control = list(maxit=100000))
...
out <- list("solns" = o$par[1:3], "pos" = pos, "objMax" = -o$value, "a" = al, "wb" = WB, "yInit" = y0, "zInit" = z0, "winProb" = win)
return(out)
}
