...
\institute{\inst{1}Syracuse University \\ \url{kbuzard@syr.edu} \and 
\inst{2}UC San Diego}
...
\pause
FOC look the same for Vote Buyer B, e.g. for $b(0)$:
\pause
	\[
		\frac{e^{-Y} + e^{-Z}}{\left(1+e^{-Y}\right)\left(1+e^{-Z}\right)} \frac{e^{-X}}{\left(1+e^{-X}\right)^2}= \frac{1}{W_B}
	\]
...
\vskip.1in
\pause
For Vote Buyer A, FOC for $a(0)$:
\pause
	\[
		\frac{e^{-Y} + e^{-Z}}{\left(1+e^{-Y}\right)\left(1+e^{-Z}\right)} \frac{e^{-X}}{\left(1+e^{-X}\right)^2}= \frac{1}{W_A}
	\]
	\vskip-.15in
	\pause
		\begin{itemize}
			\item Similar to $B$, except only smaller root satisfies SOC
		\end{itemize}
