					...
					\item I'm going to start with linear parameterization of $v(z)$ and add uncertainty. For now, take same form for $v(z)$
						...
						and take $z$ as a random variable distributed normally with mean $z$ and standard deviation $\sigma_z$. This is unfortunate nomenclature, since later we'll probably want to take this to a standard normal ``Z'' 
							\begin{itemize}
								...
								\item Uncertainty comes from: log-rolling and cross-pressuring; length of voting record; electoral incentives that we can't control for
							\end{itemize}
				\end{itemize}
		\end{itemize}
\end{itemize}
							...
							\item The FOC for each $j$ is then
								...
								as long as the constraint is satisfied.
									\begin{itemize}
										...
										\item If the constraint is not binding, vote buyer B will pay every single legislator (we could put in some kind of fixed cost of dealing with each legislator to short circuit this outcome)
										\item Perhaps there's something in equilibrium I'm not seeing yet that will reduce the number of legislators that are paid
									\end{itemize}
						\end{itemize}
				\end{itemize}
		\end{itemize}	
