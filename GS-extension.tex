...
\begin{itemize}
	\item Imagine a see-saw; these guys are weights on the see-saw.
\end{itemize}
...
\begin{tabular}{ccccc}
		...
		\item It is NOT the case that $b(0)$ has to be such that $X=Y$ even though $b\left(\frac{1}{2}\right) =0$. In fact, it seems in numerical examples that this does \textit{not} happen.
			\begin{itemize}
				\item However, I do see that this existence of the one non-negative bribe is not common. It exists for $\al = 0$ and WB = 8, but goes away as soon as $\al = 0.1$ or WB = 9.
			\end{itemize}
