...
Without loss of generality, assume that it is the legislators at $-\frac{1}{2}$ and $0$ that are bribed. The first order conditions with respect to the bribes for these two legislators are
	...
	\end{equation}
		...
		\vskip.2in
The second derivative, for instance with respect to $b_0$ is
\begin{equation}
  \frac{\partial^2 L}{\partial b_0^2} = W_B\left[\frac{e^{-Z} + e^{-Y}}{\left(1+e^{-Z}\right)\left(1+e^{-Y}\right)} \right] \left[ \frac{e^{-X}}{\left(1+e^{-X}\right)^3} \right] \left\{e^{-X}-1 \right\}
	\label{eq:secderiv}
\end{equation}
Everything except for the expression in the curly braces is positive. The expression in the curly braces is positive when $X<0$ and negative when $X>0$. It is zero when $X=0$. Analogous expressions hold for the second derivatives with respect to $b_{-.5}$ and $b_{.5}$.

\vskip.2in
The cross partials are of the form 
\begin{equation}
  \frac{\partial^2 L}{\partial b_{-.5} \partial b_0} = W_B\left[\frac{e^{-X}}{\left(1+e^{-X}\right)^2\left(1+e^{-Z}\right)^2} \right] \left[ \frac{e^{-Y}}{\left(1+e^{-Y}\right)^2} \right] \left\{e^{-2Z} - 1 \right\}
	\label{eq:xpartial}
\end{equation}
...
When one non-negativity constraint binds, the bordered Hessian is 4x4: \\
		$\left|\begin{tabular}{cccc}
			0 & $\frac{\partial g_3}{\partial b_{-.5}}$ & $\frac{\partial g_3}{\partial b_0}$ & $\frac{\partial g_3}{\partial b_{.5}}$ \\
			$\frac{\partial g_3}{\partial b_{-.5}}$ & $\frac{\partial^2 L}{\partial b_{-.5}^2}$ & $\frac{\partial^2 L}{\partial b_0\partial b_{-.5}}$ & $\frac{\partial^2 L}{\partial b_{.5} \partial b_{-.5}}$  \\
			$\frac{\partial g_3}{\partial b_0}$ & $\frac{\partial^2 L}{\partial b_{-.5} \partial b_0}$ & $\frac{\partial^2 L}{\partial b_0^2}$ & $\frac{\partial^2 L}{\partial b_{.5}\partial b_0}$ \\
			$\frac{\partial g_3}{\partial b_{.5}}$ & $\frac{\partial^2 L}{\partial b_{-.5}\partial b_{.5}}$ & $\frac{\partial^2 L}{\partial b_0 \partial b_{.5}}$ & $\frac{\partial^2 L}{\partial b_{.5}^2}$
		\end{tabular} \right|\\$

Evaluating the constraint terms, we have\\
$\left|\begin{tabular}{cccc}
			0 & 0 & 0 & -1 \\
			0 & $\frac{\partial^2 L}{\partial b_{-.5}^2}$ & $\frac{\partial^2 L}{\partial b_0\partial b_{-.5}}$ & $\frac{\partial^2 L}{\partial b_{.5} \partial b_{-.5}}$  \\
			0 & $\frac{\partial^2 L}{\partial b_{-.5} \partial b_0}$ & $\frac{\partial^2 L}{\partial b_0^2}$ & $\frac{\partial^2 L}{\partial b_{.5}\partial b_0}$ \\
			-1 & $\frac{\partial^2 L}{\partial b_{-.5}\partial b_{.5}}$ & $\frac{\partial^2 L}{\partial b_0 \partial b_{.5}}$ & $\frac{\partial^2 L}{\partial b_{.5}^2}$
		\end{tabular} \right|$

\vskip.2in
The second order condition is on the last two principal minors. The largest must be non-positive. The next to last should then be non-negative.

The next-to-last principal minor is the 3x3 matrix that is the upper-lefthand corner of this matrix. This determinant should be non-negative but is identically equal to zero because of the first row of zeros. This therefore provides no restrictions on the problem.\footnote{In the case where it is the non-negativity constraint on either $b_0$ or $b_{-.5}$ that binds, this condition \textit{does} imply an extra constraint. When $b_0 = 0$, the extra constraint is that $\frac{\partial^2 L}{\partial b_{-.5}^2} \geq0$, i.e. $Y\leq 0$. When $b_{-.5}=0$, the extra condition is that $X\leq 0$.\label{fn:extra}}
...
In this case where the non-negativity constraint binds for the variable associated with $Z$, that is $b_\frac{1}{2}$, there are three possibilities, $Z=0$, $Z>0$ and $Z<0$. I will take each in turn.
\begin{enumerate}
	\item $Z = 0$, $e^Z = 1$. Then the only values of $X$ and $Y$ with which both Equation~\ref{eq:1} and \ref{eq:2} are consistent are $X=Y$.\footnote{Note that this is the only case in which $e^{-2Z} =1$, i.e. $\frac{\partial^2 L}{\partial b_{-\frac{1}{2}} \partial b_0} = 0$. In the other two cases, the right hand side of \ref{eq:soc} is strictly positive so that the two second derivatives on the left hand side must be either both positive or both negative. This means that $X$ and $Y$ must be either both positive or both negative.}
	\item $Z > 0$, $e^Z > 1$. In this case, $X$ and $Y$ must be positive since they are greater than $Z$. Equation~\ref{eq:1} is consistent only with $Y\geq X\geq 0$, while Equation~\ref{eq:2} is consistent with $X \geq Y \geq 0$. Since both equations must hold, we have $X=Y \geq 0$.
	\item $Z < 0$, $e^Z < 1$. In this case, Equation~\ref{eq:1} is consistent with $X\geq Y\geq 0$ or $0 \geq Y \geq X$. Equation~\ref{eq:2} is consistent with $Y \geq X \geq 0$ or $0 \geq X \geq Y$. Since both equations must hold, we have either $X=Y \geq 0$ or $X=Y \leq 0$.
\end{enumerate}
		In any case, it must be that $X = Y$. Similar, slightly simpler proofs go through when the zero bribe is for either of the other two legislators because of the extra condition described in Footnote~\ref{fn:extra}. $\hfill\blacksquare$
