...
\begin{itemize}
   ...
   Equation~\ref{eq:twoways} can be rearranged into two similar forms. First:
	  ...
	  \begin{equation}
		...
		\end{equation}
	\end{itemize}	
In this case where the non-negativity constraint binds for the variable associated with $Z$, that is $b\left(\frac{1}{2}\right)$, there are two possibilities. If $Z < 0$, $e^Z < 1$. In this case,
	\begin{itemize}
		\item Equation~\ref{eq:1} is consistent with $X\geq Y\geq 0$, $X\geq 0 \geq Y$, or $0 \geq X \geq Y$.
		\item Equation~\ref{eq:2} is consistent with $Y \geq X \geq 0$, $Y \geq 0 \geq X$, or $0 \geq Y \geq X$.
	\end{itemize}
	
If $Z > 0$, $e^Z < 1$. In this case, ...
	
The second order conditions will pin down the solution in this case of two non-negative bribes.

\begin{itemize}
	\item SOCs imply that $X$ and $Y$ are either both positive, both negative, or one zero and the other unrestricted. But we can use these inequalities to get rid of the last case [finish this]
\end{itemize}
...
\begin{itemize}
	\item When $Z > 0$ (as is the case as long as $\al > -0.5$), this expression is negative
\end{itemize}
...
\begin{itemize}
  ...
  We need this determinant to be non-positive, that is
		...
		\]
	  The right hand side is non-negative because it's a square. The left hand side must then be non-negative. , and since we know that one term is positive from the previous principal minor result, the other must be positive as well. Thus both $X$ and $Y$ must be positive.
\end{itemize}
